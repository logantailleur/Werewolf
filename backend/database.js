...
async function assignRoleToPlayer(gameCode, playerID, role) {
    return new Promise((resolve, reject) => {
        let sql = 'UPDATE player SET role = ? WHERE game_code = ? AND player_id = ?';
...
function joinGame(gameCode, playerID, playerName) {
    return new Promise((resolve, reject) => {
        let gameOpenQuery = 'SELECT started, num_players FROM game WHERE game_code = ?';
        db.get(gameOpenQuery, [gameCode], (err, row) => {
            if (err) {
                ...
                db.run(playerInsert, [gameCode, playerID, playerName], function (err) {
                    if (err) {
                        reject({ success: false, message: 'Failed to add new player: ' + err.message });
                    } else {
                        const gameUpdate = 'UPDATE game SET num_players = num_players + 1 WHERE game_code = ?';
                        db.run(gameUpdate, [gameCode], function (err) {
                            if (err) {
                                reject({ success: false, message: 'Failed to update player count: ' + err.message });
                            } else {
                                resolve({ success: true, message: 'Successfully added player.' });
                            }
                        });
                    }
                });
...
function getRole(gameCode, playerID) {
    return new Promise((resolve, reject) => {
        let roleQuery = 'SELECT role FROM player WHERE game_code = ? AND player_id = ?';
